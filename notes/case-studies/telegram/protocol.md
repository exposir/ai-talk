...
| 现有协议        | 问题                                |
| --------------- | ----------------------------------- |
| HTTPS           | 太重，不适合移动网络频繁断开/重连   |
| Signal Protocol | 2013 年还没普及，且只管加密不管传输 |
...
| 概念                  | 类比             | 作用                       |
| --------------------- | ---------------- | -------------------------- |
| **Authorization Key** | 你家的门锁钥匙   | 证明"我是我"，几乎永不更换 |
| **Message Key**       | 每封信的密封贴纸 | 每条消息不同，防篡改       |
| **Session**           | 一次对话会话     | 支持多设备、断线重连       |

### 阅读建议

| 你的背景         | 推荐阅读顺序                         |
| ---------------- | ------------------------------------ |
| **产品/运营**    | 简介 → 云端 vs 端到端加密 → 安全争议 |
| **前端开发**     | 简介 → 传输层 → 消息结构             |
| **后端/安全**    | 全部按顺序阅读                       |
| **密码学爱好者** | 直接跳到加密算法 → IGE 模式          |
...
| 组件             | 状态    | 说明                                               |
| ---------------- | ------- | -------------------------------------------------- |
| **协议规范**     | ✅ 公开 | 任何人可阅读并实现自己的客户端                     |
| **官方客户端**   | ✅ 开源 | iOS/Android/Desktop 均在 GitHub                    |
| **TDLib 核心库** | ✅ 开源 | [github.com/tdlib/td](https://github.com/tdlib/td) |
| **服务端代码**   | ❌ 闭源 | 从未公开                                           |

> **信息来源**：本文内容主要来自
> [Telegram 官方 MTProto 文档](https://core.telegram.org/mtproto)，包括协议描述、密钥创建、传输协议等子页面。
...
| 特性           | 说明                                             |
| -------------- | ------------------------------------------------ |
| **错误传播**   | 一个密文块错误会影响后续所有明文块（"无限扩散"） |
| **无并行化**   | 加密/解密必须串行，无法利用多核                  |
| **无内置认证** | 不提供完整性校验，需额外 MAC                     |

#### 为什么 MTProto 选择 IGE 而非 GCM？

| 对比项           | IGE (MTProto)      | GCM (业界主流)      |
| ---------------- | ------------------ | ------------------- |
| **认证方式**     | 解密后校验 SHA-256 | 加密时生成 GMAC     |
| **错误检测**     | 解密后才知道       | 可提前拒绝无效密文  |
| **性能**         | 串行，无硬件加速   | 并行，AES-NI 加速   |
| **Nonce 敏感性** | 不依赖 Nonce       | Nonce 重用导致灾难  |
| **标准化**       | 非标准             | TLS 1.3、IPsec 采用 |

**Telegram 的辩护**：

1. **消息内容绑定密钥**：`msg_key` 依赖于明文内容，使得自适应攻击无法进行
2. **解密后全面校验**：验证 salt、session_id、msg_id、长度、时间戳
3. **历史原因**：MTProto 设计于 2013 年，当时 GCM 尚未普及
...
| 字段            | 长度    | 说明                             |
| --------------- | ------- | -------------------------------- |
| **server_salt** | 64-bit  | 防重放，由服务器分配             |
| **session_id**  | 64-bit  | 客户端生成，标识会话             |
| **msg_id**      | 64-bit  | 时间戳×2^32 + 序号，必须递增     |
| **seq_no**      | 32-bit  | 内容相关消息计数器×2 + 1（或×2） |
| **msg_len**     | 32-bit  | msg_data 的字节长度              |
| **msg_data**    | 变长    | TL 序列化的消息内容              |
| **padding**     | 12-1024 | 随机填充，使总长度被 16 整除     |
...
MTProto 通过**临时密钥**机制实现前向保密（Perfect Forward
Secrecy），确保即使长期密钥泄露，历史通信也不可解密。
...
1. 客户端创建临时 `auth_key`（设置 `expires_in`）
2. 使用 `auth.bindTempAuthKey` 将临时密钥绑定到永久密钥
3. 所有消息使用临时密钥加密
4. 临时密钥过期后，重新生成并绑定
5. 旧的临时密钥从服务器 RAM 中删除
...
| 触发条件                     | 操作               |
| ---------------------------- | ------------------ |
| 加密/解密超过 **100 条消息** | 自动发起 Re-keying |
| 密钥使用超过 **1 周**        | 自动发起 Re-keying |
| 任意一方主动请求             | 协商新密钥         |
...
| 应用         | 加密协议         | 默认 E2E          | 服务器可读      | 开源        | 量子安全 | 特点                   |
| ------------ | ---------------- | ----------------- | --------------- | ----------- | -------- | ---------------------- |
| **Telegram** | MTProto 2.0      | ❌ 仅 Secret Chat | ✅ 普通聊天可读 | 规范公开    | ❌       | 移动优化，自研         |
| **WhatsApp** | Signal Protocol  | ✅                | ❌ 无法读取     | ❌ 闭源实现 | ❌       | 业界标准，Meta 旗下    |
| **Signal**   | Signal Protocol  | ✅                | ❌ 无法读取     | ✅ 完全开源 | ✅ PQXDH | 隐私标杆，非营利       |
| **iMessage** | PQ3              | ✅ 仅 Apple 设备  | ❌ 无法读取     | ❌          | ✅ Kyber | 量子安全等级最高       |
| **微信**     | MMTLS (魔改 TLS) | ❌                | ✅ 完全可读     | ❌          | ❌       | 服务器可读，无前向保密 |
| **Line**     | Letter Sealing   | ✅ 可选           | ⚠️ 未开启时可读 | ❌          | ❌       | 日本/东南亚流行        |
| **Discord**  | TLS 1.3          | ❌                | ✅ 完全可读     | ❌          | ❌       | 仅传输加密，服务器可读 |

### 各协议详解

#### Signal Protocol（WhatsApp / Signal / Facebook Messenger）

**业界黄金标准**，被 WhatsApp、Facebook Messenger（Secret
Conversations）等采用。

| 特性           | 说明                                  |
| -------------- | ------------------------------------- |
| **双棘轮算法** | 每条消息一个密钥，前向保密 + 后向安全 |
| **X3DH**       | 异步密钥交换，支持离线消息            |
| **Curve25519** | 椭圆曲线加密                          |
| **开源**       | 完全公开，经大量审计                  |
| **量子安全**   | 2024 年推出 PQXDH + SPQR（三重棘轮）  |

**优势**：经过学术验证，广泛采用，不断进化
**劣势**：WhatsApp 闭源实现，备份默认不加密
...
| 特性           | 说明                                           |
| -------------- | ---------------------------------------------- |
| **安全等级**   | Level 3（最高），超越 Signal                   |
| **抗量子算法** | NIST 批准的 Kyber 算法                         |
| **密钥轮换**   | 每 50 条消息或 7 天轮换一次                    |
| **限制**       | 仅 Apple 设备间有效，与 Android 通信退化为 SMS |
...
| **端到端加密** | ❌ 服务器可解密所有消息             |
| **前向保密**   | ❌ 缺失                             |
| **初始化向量** | 使用确定性 IV（违反密码学最佳实践） |
| **元数据保护** | ❌ 用户 ID、请求 URI 暴露           |
| **政府访问**   | ✅ 依法配合中国政府数据请求         |
...
| ------------ | ----------------------- | ------------------------- |
| **设计年份** | 2013 年                 | 2013 年                   |
| **默认 E2E** | ❌ 仅 Secret Chat       | ✅ 所有消息               |
| **开源程度** | 规范公开，服务端闭源    | 完全开源                  |
| **加密模式** | AES-256-IGE             | AES-256-GCM               |
| **密钥交换** | DH + RSA                | X3DH (Extended Triple DH) |
| **棘轮算法** | 简化版（每 100 条消息） | 双棘轮（每条消息）        |
| **量子安全** | ❌                      | ✅ PQXDH（2024）          |
| **学术认可** | 形式化验证通过          | 多篇论文分析              |
...
> **Level 3**：抵御当前和未来量子计算攻击 + 密钥泄露后恢复 **Level
> 2**：抵御量子攻击 **Level 1**：传统端到端加密 **Level 0**：仅传输加密或无加密
...
| 年份     | 论文                                                                 | 主要发现                                              |
| -------- | -------------------------------------------------------------------- | ----------------------------------------------------- |
| **2017** | _Automated Symbolic Verification of Telegram's MTProto 2.0_ (Sec'17) | 形式化验证证明 MTProto 2.0 满足 IND-CCA 安全性        |
| **2021** | _A Formal Security Analysis of the Signal Messaging Protocol_        | Signal 协议的对比分析，指出 MTProto 的非默认 E2E 问题 |
| **2022** | _Analysis of MTProto's Encryption Scheme_                            | 指出 IGE 模式的潜在风险，建议迁移到 AEAD              |

### 外部评估

- [Crypto 101](https://www.crypto101.io/) - 密码学入门，解释 IGE 等模式
- [Moxie Marlinspike 对 Telegram 的评论](https://twitter.com/maboroshi) -
  Signal 创始人的批评视角
- [ETH Zurich 安全分析](https://mtpsym.github.io/) - MTProto 的符号验证项目
