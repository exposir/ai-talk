...
- 现有协议：HTTPS；问题：握手与重连成本较高，不利于频繁断开/重连
- 现有协议：Signal Protocol；问题：早期生态不成熟，且主要聚焦端到端加密而非传输层优化
...
- **RSA 加密**：`new_nonce` 通过 RSA 加密传输，降低中间人风险
...
3. 如果 `salt` 过期，服务器返回 `bad_server_salt` 并提供新值
...
> **WebSocket 传输常与混淆一起使用，具体以官方实现为准**
...
- **长度**：128 位
- **计算方式**：由 `auth_key` 与明文消息计算的 SHA-256 中间 128 位
...
1. **消息内容绑定密钥**：`msg_key` 依赖于明文内容，增加篡改检测难度
2. **解密后校验**：验证 salt、session_id、msg_id、长度、时间戳
3. **历史原因**：早期实现背景下，GCM 的生态与部署经验尚不充分
...
> 以下为公开资料的高层概览，细节可能随时间变化或因版本/地区而异。

- 应用：**Telegram**；加密协议：MTProto 2.0；默认 E2E：❌ 仅 Secret Chat；服务器可读：✅
普通聊天可读；开源：协议/客户端公开，服务端闭源；量子安全：暂无官方声明；特点：移动优化，自研
- 应用：**WhatsApp**；加密协议：Signal Protocol；默认 E2E：✅；服务器可读：❌；开源：协议公开，实现部分闭源；量子安全：暂无官方声明；特点：大规模部署
- 应用：**Signal**；加密协议：Signal Protocol；默认 E2E：✅；服务器可读：❌；开源：✅；量子安全：研究与规划中；特点：隐私优先
- 应用：**iMessage**；加密协议：PQ3（公开白皮书）；默认 E2E：✅ 仅 Apple 设备；服务器可读：❌；开源：❌；量子安全：✅（以官方说明为准）
- 应用：**微信**；加密协议：MMTLS；默认 E2E：❌；服务器可读：✅；开源：❌；量子安全：暂无公开信息
- 应用：**Line**；加密协议：Letter Sealing；默认 E2E：✅ 可选；服务器可读：⚠️
未开启时可读；开源：❌；量子安全：暂无公开信息
- 应用：**Discord**；加密协议：TLS（传输层）；默认 E2E：❌；服务器可读：✅；开源：❌；量子安全：暂无公开信息
...
- 特性：**开源**；说明：协议与实现公开，经广泛审计
- 特性：**量子安全**；说明：社区有 PQXDH 等提案与研究
...
- 对比项：**开源程度**；MTProto 2.0：规范公开，服务端闭源；Signal Protocol：协议与实现公开
- 对比项：**加密模式**；MTProto 2.0：AES-256-IGE；Signal Protocol：AEAD（如 AES-GCM/ChaCha20-Poly1305）
- 对比项：**密钥交换**；MTProto 2.0：DH + RSA；Signal Protocol：X3DH
- 对比项：**棘轮算法**；MTProto 2.0：简化轮换；Signal Protocol：双棘轮
- 对比项：**量子安全**；MTProto 2.0：暂无官方声明；Signal Protocol：研究与规划中
- 对比项：**学术分析**；MTProto 2.0：存在形式化分析与争议；Signal Protocol：多篇论文分析
...
- 有学术工作在特定假设下分析 MTProto 2.0 的安全性
- 端到端加密通过形式化验证
