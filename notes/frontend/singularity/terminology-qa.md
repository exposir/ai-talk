...
### 7.7 Signal 和 React 并发模式能兼容吗？这是最大的技术风险吗？

**答**：是的，这是项目成败的关键技术风险。Signal 的细粒度更新与 React Concurrent Mode 存在天然冲突：

**问题本质**：
- Signal 直接更新订阅者，绕过 React 调度器
- React 18+ 的 `useTransition`/`Suspense` 依赖调度器控制
- 直接更新可能导致 Tearing（渲染撕裂）

**解决方案**：强制使用 `useSyncExternalStore`
- 这是 React 18 官方推荐的外部状态订阅方式
- 代价是回退到同步渲染（de-opt from time-slicing）
- 通过 Selector 细粒度订阅弥补性能损耗

**结论**：技术上可解决，但需要在 M0 阶段充分验证。若发现无法兼容，需切换技术方案。

### 7.8 为什么 CRDT 协作层要推迟到 v1.1？

**答**：CRDT 协作层（atomSync）是整个项目风险最高的模块，原因如下：

1. **包体积风险**：Yjs 约 30-40KB (gzipped)，可能显著增加核心包体积
2. **复杂度风险**：CRDT 合并语义复杂，调试困难，边界案例多
3. **性能风险**：多人协作场景下的性能验证需要大量测试
4. **时间风险**：在有限时间内同时完成 Core + CRDT 不现实

**策略**：
- v1.0 聚焦 Core + Async + Machine，确保基础稳定
- v1.1 作为独立发布，专注协作层
- 若 v1.1 验证失败（体积/性能/稳定性），推迟到 v2

### 7.9 如果 TanStack Store 发布了，这个项目还有意义吗？

**答**：需要持续跟踪 TanStack Store（Tanner Linsley 即将发布）的进展。评估标准：

| 如果 TanStack Store... | 决策 |
|:----------------------|:-----|
| 覆盖 80%+ 目标且生态成熟 | 考虑放弃，转为贡献者 |
| 只做 Core，不做 Machine/Sync | 继续，专注差异化能力 |
| API 设计与奇点冲突 | 评估迁移成本，决定是否合并 |

**核心原则**：不重复造轮子。如果社区已有成熟方案，应积极拥抱而非对抗。
