...
> 把我当傻子，一步步教我连接 Supabase 并实现 Google
> OAuth 登录。假设我从未用过 Supabase，从注册账号开始讲起。
...
L3 检查 → 新创建的文件（supabase.js, auth.js, AuthContext.jsx）头部是否添加
[INPUT]/[OUTPUT]/[POS] 注释？L2 检查 →
lib/CLAUDE.md 和 contexts/CLAUDE.md 是否记录新增文件？L1 检查 → 项目根目录 CLAUDE.md 是否更新技术栈（添加 Supabase）和目录说明？
